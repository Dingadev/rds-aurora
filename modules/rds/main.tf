...
locals {
  ...
  # Reference for database names https://docs.aws.amazon.com/AmazonRDS/latest/APIReference/API_CreateDBInstance.html
  default_parameter_group_name = {
    ...
    oracle-se2     = "default.${var.engine}-${replace(var.engine_version, "/(.+?)\\.(\\d).*?\\..+/", "-$1.$2")}"
    oracle-ee      = "default.${var.engine}-${replace(var.engine_version, "/(.+?)\\.(\\d).*?\\..+/", "-$1.$2")}"
    oracle-ee-cdb  = "default.${var.engine}-${replace(var.engine_version, "/(.+?)\\.(\\d).*?\\..+/", "-$1.$2")}"
    oracle-se2-cdb = "default.${var.engine}-${replace(var.engine_version, "/(.+?)\\.(\\d).*?\\..+/", "-$1.$2")}"
  }
...
resource "aws_db_instance" "primary" {
  ...
  vpc_security_group_ids = concat(
    [aws_security_group.db.id],
    var.additional_primary_instance_security_group_ids,
  )
...
resource "aws_db_instance" "replicas" {
  ...
  vpc_security_group_ids = concat(
    [local.read_replica_security_group_id],
    var.additional_read_replica_instance_security_group_ids,
  )
